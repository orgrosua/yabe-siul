...
class Asset
{
    ...
    {
        if (static::$manifest === []) {
            $manifest = file_get_contents(dirname(SIUL::FILE) . '/build/manifest.json');
    ...
    public static function read_entrypoints(): array
    {
        if (static::$entrypoints === []) {
            $entrypoints = file_get_contents(dirname(SIUL::FILE) . '/build/entrypoints.json');
        ...
        if (isset($entrypoints[$key])) {
            ...
            if (isset($entry['js'])) {
                foreach ($entry['js'] as $js) {
                     ...
                     */
                    $dependency = apply_filters('f!yabe/siul/asset:enqueue_entry.dependency', $dependency, $manifest_key, $key);
    ...
    public static function register_script(string $key, array $deps = [], string $src = '', bool $in_footer = false)
    {
        ...
        if ($src === '') {
            ...
            if (isset($manifest[$key])) {
                $src = strncmp($manifest[$key], 'http', strlen('http')) === 0 ? $manifest[$key] : plugins_url('build/' . $manifest[$key], SIUL::FILE);
            } else {
                return false;
            }
        }

        $is_registered = wp_register_script($handle, $src, $deps, SIUL::VERSION, $in_footer);
    ...
    public static function register_style(string $key, array $deps = [], string $src = '', string $media = 'all')
    {
        ...
        $is_registered = wp_register_style($handle, $src, $deps, SIUL::VERSION, $media);
    ...
    {
        return plugins_url('build/', SIUL::FILE);
    }
}
